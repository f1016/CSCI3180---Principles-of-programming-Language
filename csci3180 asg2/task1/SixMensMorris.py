...
class SixMensMorris():
    ...
    def start_game(self):
        ...
        while not end:
            ...
            if self.num_play > 12 and self.check_win(player): # check if the 'cant move' case winning
                end = True
            else:
                if self.board.form_mill(x, player):
                    print('You form a mill!')
                    ###TODO
                    player.next_remove(self.opponent(player)) # remove
                    self.board.print_board()

                if self.num_play > 12 and self.check_win(player): # check if oppnenent remain 2 mens
                    end = True
