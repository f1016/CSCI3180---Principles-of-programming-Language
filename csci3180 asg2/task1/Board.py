...
class Board():
    ...
    def print_board(self):
        ...
        print('|   ', ' ' * 6, '|', ' ' * 6, '   |', end='    ');
        print('|   ', ' ' * 6, '|', ' ' * 6, '   |')
        print('|   ', color(self.state[3]), '-' * 4, color(self.state[4]), '-' * 4, color(self.state[5]), '   |',
              end='    ');
        print('|   ', 'd', '-' * 4, 'e', '-' * 4, 'f', '   |')
        print('|   ' * 2, ' ' * 7, '   |' * 2, end='    ');
        print('|   ' * 2, ' ' * 7, '   |' * 2)
        print(color(self.state[6]), '-', color(self.state[7]), ' ' * 13, color(self.state[8]), '-',
              color(self.state[9]), end='    ');
        print('g', '-', 'h', ' ' * 13, 'i', '-', 'j', )
        print('|   ' * 2, ' ' * 7, '   |' * 2, end='    ');
        print('|   ' * 2, ' ' * 7, '   |' * 2)
        print('|   ', color(self.state[10]), '-' * 4, color(self.state[11]), '-' * 4, color(self.state[12]), '   |',
              end='    ');
        print('|   ', 'k', '-' * 4, 'l', '-' * 4, 'm', '   |')
        print('|   ', ' ' * 6, '|', ' ' * 6, '   |', end='    ');
        print('|   ', ' ' * 6, '|', ' ' * 6, '   |')
    ...
    def check_move(self, s, t, player):
        ...
        for j, k in self.edges:
            if (j == s and k == t) or (j == t and k == s):
                adj_pos = True

        if valid_flag != False and (not adj_pos or self.state[s] != player.get_symbol() or self.state[t] != '.'):
            valid_flag = False
    ...
    def check_remove(self, pos, player):
        ...
        ### TODO
        if valid_flag != False and (self.state[pos] != player.get_symbol() or self.state[pos] == '.'):
            valid_flag = False
    ...
    def form_mill(self, pos, player):
        ...
        for i in range(len(self.mills)):
            for j in range(3):
                if self.mills[i][j] == pos:  # finding which combination can be form
                    if (self.state[self.mills[i][0]] == self.state[self.mills[i][1]]
                            and self.state[self.mills[i][1]] == self.state[self.mills[i][2]]):
                        if_form = True
            ...
            for i in range(len(self.state)):
                if self.state[i] == opponent.get_symbol():
                    ...
                    for j, k in self.edges:
                        ...
                        if i == j and self.state[k] == '.':
                            piece_can_move = True
                        elif i == k and self.state[j] == '.':
                            piece_can_move = True
