...
public class Task4Map {
  ...
  public void addObject(Object object) {
    ...
    if (object instanceof Task4Monster[]) {
      for (Task4Monster task4Monster : ((Task4Monster[])object)) {
        pos = task4Monster.getPos();
        this.cells[pos.getRow() - 1][pos.getColumn() - 1].setOccupiedObject(task4Monster);
      }
    } else {
      ...
      } else if (object instanceof Task4Merchant) { // if it is Merchant
    ...
    for (int i = 0; i < 7; i++) {
      ...
      for (int j = 0; j < 7; j++) {
        ...
        if (occupiedObject != null) {
          ...
          } else if (occupiedObject instanceof Task4Merchant) { // if it is merchant
  ...
  public void update(Task4Soldier task4Soldier, int oldRow, int oldColumn, int newRow, int newColumn) {
    this.cells[oldRow - 1][oldColumn - 1].setOccupiedObject(null);
    this.cells[newRow - 1][newColumn - 1].setOccupiedObject(task4Soldier);
  }
}